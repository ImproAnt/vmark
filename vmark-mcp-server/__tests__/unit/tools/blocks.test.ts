...
describe('Block Tools', () => {
  ...
  describe('block_toggle', () => {
    it('should toggle blockquote', async () => {
      const result = await server.callTool('block_toggle', { type: 'blockquote' });

      expect(result.success).toBe(true);
      expect(result.content[0].text).toContain('Toggled blockquote block');
    });

    it('should toggle heading with level', async () => {
      const result = await server.callTool('block_toggle', {
        type: 'heading',
        level: 1,
      });

      expect(result.success).toBe(true);
      expect(result.content[0].text).toContain('Toggled heading block');
    });

    it('should reject invalid block type', async () => {
      const result = await server.callTool('block_toggle', { type: 'unknown' });
    ...
    it('should require level for heading', async () => {
      const result = await server.callTool('block_toggle', { type: 'heading' });

      expect(result.success).toBe(false);
      expect(result.content[0].text).toContain('level is required');
    });

    it('should validate heading level range', async () => {
      const result = await server.callTool('block_toggle', {
        type: 'heading',
        level: 10,
      });
    ...
    it('should send correct bridge request', async () => {
      await server.callTool('block_toggle', { type: 'blockquote' });

      const requests = bridge.getRequestsOfType('block.toggle');
      expect(requests).toHaveLength(1);
      expect(requests[0].request).toMatchObject({
        type: 'block.toggle',
    ...
    it('should handle bridge errors', async () => {
      bridge.setNextError(new Error('Editor read-only'));

      const result = await server.callTool('block_toggle', { type: 'paragraph' });

      expect(result.success).toBe(false);
      expect(result.content[0].text).toContain('Editor read-only');
    });
  });
