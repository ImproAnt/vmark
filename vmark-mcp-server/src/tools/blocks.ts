...
export function registerBlockTools(server: VMarkMcpServer): void {
  ...
  // block_toggle - Toggle a block type
  server.registerTool(
    {
      name: 'block_toggle',
      description:
        'Toggle a block type at the current cursor position. ' +
        'If the block is already the specified type, converts back to paragraph.',
      inputSchema: {
        type: 'object',
        properties: {
          type: {
            type: 'string',
            enum: [...VALID_BLOCK_TYPES],
            description: 'The block type to toggle.',
          },
          level: {
            type: 'number',
            description: 'Level for headings (1-6). Only used when type is "heading".',
    ...
    async (args) => {
      ...
      try {
        ...
        return VMarkMcpServer.successResult(`Toggled ${type} block`);
      } catch (error) {
        return VMarkMcpServer.errorResult(
          `Failed to toggle block: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
